...
function makeCompute(SHOTS: RawShot[], SESSIONS: Session[], sessState: SessState) {
  ...
  // engine-computed mean trajectory for the average-shot views
  function meanFlight(club: string) {
    const v = SHOTS.filter((x) => x.club === club && x.bs != null && x.la != null && x.spin != null);
  ...
  return { series, classify2, clubHealth, carryAttribution, measuredCarryChange, meanFlight };
}
...
// Average-shot views (side + top-down) — ported geometry from flightViews()
function FlightViews({
  club,
  color,
  result,
}: {
  club: string;
  color: string;
  result: ReturnType<ReturnType<typeof makeCompute>['meanFlight']>;
}) {
  // animated ball positions (one per view), driven by RAF at real flight speed
  ...
  // animate ball at real flight speed, loop with a hold, clean up on unmount
  useEffect(() => {
    if (!built) {
      setSide(null);
      setTop(null);
...
export default function Trends() {
  ...
  if (!current) {
    ...
    if (sessForClub.length < 2) {
      ...
      const flight = compute.meanFlight(current!);
      ...
      content = (
        <>
          ...
          {/* average shot */}
          {flight && <FlightViews club={current!} color={color} result={flight} />}
